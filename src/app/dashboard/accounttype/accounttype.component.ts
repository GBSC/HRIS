...
import { DeleteAccountTypeComponent } from './dialogs/delete/delete.component'; 
...
export class AccountTypeComponent implements OnInit {
  ...
  addNewAccountType(actype: Accounttype) {
    const dialogRef = this.dialog.open(AddAccountTypeDialogComponent, {
      data: {actype: actype }
    });
  ...
  AccounttypeEdit(i: number, Id: number, Code: string, Name: string, Description: string) {
    this.id = Id;
  ...
  deleteAccountType(i: number, Id: number, Code: string, Name: string, Description: string) {
    this.index = i;
    this.id = Id;
...
export class ExampleDataSource extends DataSource<Accounttype> {
  ...
  connect(): Observable<Accounttype[]> {
    ...
    this._exampleDatabase.getAllAccounttypes();

    return Observable.merge(...displayDataChanges).map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.accounttypedata.slice().filter((actype: Accounttype) => {
        const searchStr = (actype.Id + actype.Name + actype.Code).toLowerCase();
  ...
  sortData(data: Accounttype[]): Accounttype[] {
