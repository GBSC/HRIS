...
export class GroupComponent implements OnInit {
  ...
  EditGrup(i: number, Id: number, Code: string, Name: string, Description: string) {
    this.id = Id;
  ...
  deleteGrup(i: number, Id: number, Code: string, Name: string, Description: string) {
this.index = i;
    this.id = Id;
  const dialogRef = this.dialog.open(DeleteGroupsComponent, {
       data: {Id: Id,Code:Code, Name: Name, Description: Description}
     });
...
export class ExampleDataSource extends DataSource<Group> {
  ...
  connect(): Observable<Group[]> {
    ...
    return Observable.merge(...displayDataChanges).map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.Groupdata.slice().filter((group: Group) => {
        const searchStr = (group.Id + group.Name + group.Description ).toLowerCase();
