...
export class LanguageComponent implements OnInit {
  ...
  EditLangue(i: number, Id: number, Code: string, Name: string, Description: string) {
    this.id = Id;
  ...
  deleteLangue(i: number, Id: number, Code: string, Name: string, Description: string) {
   this.index = i;
     this.id = Id;
     const dialogRef = this.dialog.open(DeleteLanguagesComponent, {
       data: {Id: Id,Code:Code, Name: Name, Description: Description}
     });
...
export class ExampleDataSource extends DataSource<Language> {
  ...
  connect(): Observable<Language[]> {
    ...
    this._exampleDatabase.getAllLanguages();

    return Observable.merge(...displayDataChanges).map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.Languagedata.slice().filter((language: Language) => {
        const searchStr = (language.Id + language.Name + language.Description ).toLowerCase();
