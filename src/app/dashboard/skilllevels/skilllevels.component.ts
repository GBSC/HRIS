...
export class SkillLevelsComponent implements OnInit {
  ...
  addSkill(skilllevels: SkillLevels) {
    const dialogRef = this.dialog.open(AddskillLevelsComponent, {
  ...
  EditSkill(i: number, Id: number, Code: string, Name: string, Description: string) {
    this.id = Id;
    // index row is used just for debugging proposes and can be removed
    this.index = i;
    console.log(this.index);
    const dialogRef = this.dialog.open(EditSkillLevelsComponent, {
  ...
  deleteSkill(i: number, Id: number, Code: string, Name: string, Description: string) {
     this.index = i;
     this.id = Id;
     const dialogRef = this.dialog.open(DeleteSkillLevelsComponent, {
       data: {Id: Id,Code:Code, Name: Name, Description: Description}
     });
...
export class ExampleDataSource extends DataSource<SkillLevels> {
  ...
  filteredData: SkillLevels[] = [];
  renderedData: SkillLevels[] = [];
  ...
  connect(): Observable<SkillLevels[]> {
    ...
    this._exampleDatabase.getAllSkillLevels();

    return Observable.merge(...displayDataChanges).map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.SkillLevelsdata.slice().filter((skilllevels: SkillLevels) => {
        const searchStr = (skilllevels.Id + skilllevels.Name + skilllevels.Description ).toLowerCase();
  ...
  sortData(data: SkillLevels[]): SkillLevels[] {
