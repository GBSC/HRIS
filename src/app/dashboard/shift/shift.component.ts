...
export class ShiftComponent implements OnInit {
  ...
  addNew(shift: Shift) {
    const dialogRef = this.dialog.open(ShiftAddDialogComponent, {
      data: {shift: shift }
    });
  ...
  deleteshift(i: number, Id: number, Code: string, Name: string, Description: string) {
    this.index = i;
    this.id = Id;
...
export class ExampleDataSource extends DataSource<Shift> {
  ...
  connect(): Observable<Shift[]> {
    ...
    this._exampleDatabase.getAllShifts();

    return Observable.merge(...displayDataChanges).map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.shiftdata.slice().filter((shift: Shift) => {
        const searchStr = (shift.Id + shift.Name + shift.Code).toLowerCase();
