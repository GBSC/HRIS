...
import { DeleteestatusComponent } from './dialogs/delete/delete.component';
...
export class EmployeeStatuscomponent implements OnInit {
  ...
  addNew(employeestatus: EmployeeStatus) {
    const dialogRef = this.dialog.open(EmployeeStatusAddDialogComponent, {
  ...
  startEdit(i: number, Id: number, Code: string, Name: string, Description: string) {
    ...
    const dialogRef = this.dialog.open(EmployeeStatusEditDialogComponent, {
  ...
  deleteestatus(i: number, Id: number, Code: string, Name: string, Description: string) {
    this.index = i;
    this.id = Id;
    const dialogRef = this.dialog.open(DeleteestatusComponent, {
      data: {Id: Id,Code:Code, Name: Name, Description: Description}
    });
...
export class ExampleDataSource extends DataSource<EmployeeStatus> {
  ...
  connect(): Observable<EmployeeStatus[]> {
    ...
    this._exampleDatabase.getAllEmployeeStatus();

    return Observable.merge(...displayDataChanges).map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.Employeestatusdata.slice().filter((employeeStatus: EmployeeStatus) => {
        const searchStr = (employeeStatus.Id + employeeStatus.Name + employeeStatus.Code).toLowerCase();
  ...
  sortData(data: EmployeeStatus[]): EmployeeStatus[] {
