...
import { DeleteDegreeComponent } from './dialogs/delete/delete.component';
...
export class DegreeComponent implements OnInit {
  ...
  addNewDegree(degree: Degree) {
    const dialogRef = this.dialog.open(AddDegreeDialogComponent, {
      data: {degree: degree }
    });
  ...
  startEdit(i: number, Id: number, Code: string, Name: string, Description: string) {
    ...
    const dialogRef = this.dialog.open(EditDegreeDialogComponent, {
  ...
  deleteDegree(i: number, Id: number, Code: string, Name: string, Description: string) {
    this.index = i;
    this.id = Id;
    const dialogRef = this.dialog.open(DeleteDegreeComponent, {
      data: {Id: Id,Code:Code, Name: Name, Description: Description}
    });
...
export class ExampleDataSource extends DataSource<Degree> {
  ...
  connect(): Observable<Degree[]> {
    ...
    this._exampleDatabase.getAllDegrees();

    return Observable.merge(...displayDataChanges).map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.degreedata.slice().filter((degree: Degree) => {
        const searchStr = (degree.Id + degree.Name + degree.Code).toLowerCase();
