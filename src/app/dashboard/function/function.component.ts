...
import { DeleteFunctionComponent } from './dialogs/delete/delete.component';
...
export class FunctionComponent implements OnInit {
  ...
  addNew(funct: Function) {
    const dialogRef = this.dialog.open(FunctionAddDialogComponent, {
      data: {funct: funct }
    });
  ...
  startEdit(i: number, Id: number, Code: string, Name: string, Description: string) {
    ...
    const dialogRef = this.dialog.open(FunctionEditDialogComponent, {
  ...
  deletefunc(i: number, Id: number, Code: string, Name: string, Description: string) {
    this.index = i;
    this.id = Id;
    const dialogRef = this.dialog.open(DeleteFunctionComponent, {
      data: {Id: Id,Code:Code, Name: Name, Description: Description}
    });
...
export class ExampleDataSource extends DataSource<Function> {
  ...
  connect(): Observable<Function[]> {
    ...
    this._exampleDatabase.getAllFunctions();

    return Observable.merge(...displayDataChanges).map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.functiondata.slice().filter((funct: Function) => {
        const searchStr = (funct.Id + funct.Name + funct.Code).toLowerCase();
  ...
  sortData(data: Function[]): Function[] {
