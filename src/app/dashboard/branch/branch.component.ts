...
import { DeleteBranchComponent } from './dialogs/delete/delete.component';
...
export class BranchComponent implements OnInit {
  ...
  addNew(branch: Branch) {
    const dialogRef = this.dialog.open(BranchAddDialogComponent, {
      data: {branch: branch }
    });
  ...
  deletebranch(i: number, Id: number, Code: string, Name: string, Description: string) {
    this.index = i;
    this.id = Id;
...
export class ExampleDataSource extends DataSource<Branch> {
  ...
  connect(): Observable<Branch[]> {
    ...
    this._exampleDatabase.getAllbranches();

    return Observable.merge(...displayDataChanges).map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.branchdata.slice().filter((branch: Branch) => {
        const searchStr = (branch.Id + branch.Name + branch.Code).toLowerCase();
