...
export class GenderComponent implements OnInit {
  ...
  addgndr(Gender:Gender) {
    const dialogRef = this.dialog.open(AddGendersComponent, {
      data: {Gender: Gender }
    });
  ...
  Editgndr(i: number, Id: number, Code: string, Name: string, Description: string) {
    this.id = Id;
    // index row is used just for debugging proposes and can be removed
    this.index = i;
    console.log(this.index);
    const dialogRef = this.dialog.open(EditGendersComponent, {
  ...
  deletegndr(i: number, Id: number, Code: string, Name: string, Description: string) {
     this.index = i;
     this.id = Id;
     const dialogRef = this.dialog.open(DeleteGendersComponent, {
       data: {Id: Id,Code:Code, Name: Name, Description: Description}
     });
...
export class ExampleDataSource extends DataSource<Gender> {
  ...
  connect(): Observable<Gender[]> {
    ...
    this._exampleDatabase.GetAllGender();

    return Observable.merge(...displayDataChanges).map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.Genderdata.slice().filter((Gender: Gender) => {
        const searchStr = (Gender.Id + Gender.Code + Gender.Name + Gender.Description ).toLowerCase();
