...
export class RosterComponent implements OnInit {
  ...
  addNewRoster(roster: Roster) {
    const dialogRef = this.dialog.open(AddRosterDialogComponent, {
      data: {roster: roster }
    });
  ...
  RosterEdit(i: number, Id: number, Code: string, Name: string, Description: string) {
    this.id = Id;
    // index row is used just for debugging proposes and can be removed
    this.index = i;
    console.log(this.index);
    const dialogRef = this.dialog.open(EditRosterDialogComponent, {
  ...
  deleteRoster(i: number, Id: number, Code: string, Name: string, Description: string) {
    this.index = i;
    this.id = Id;
...
export class ExampleDataSource extends DataSource<Roster> {
  ...
  connect(): Observable<Roster[]> {
    ...
    this._exampleDatabase.getAllRosters();

    return Observable.merge(...displayDataChanges).map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.rosterdata.slice().filter((roster: Roster) => {
        const searchStr = (roster.Id + roster.Name + roster.Code).toLowerCase();
