...
import { DeleteETypeComponent } from './dialogs/delete/delete.component';
...
export class EmployeeTypes implements OnInit {
  ...
  addNew(employeetype: EmployeeType) {
  ...
  startEdit(i: number, Id: number, Code: string, Name: string, Description: string) {
    ...
    const dialogRef = this.dialog.open(EmployeeTypeEditDialogComponent, {
  ...
  deleteEType(i: number, Id: number, Code: string, Name: string, Description: string) {
    this.index = i;
    this.id = Id;
    const dialogRef = this.dialog.open(DeleteETypeComponent, {
      data: {Id: Id,Code:Code, Name: Name, Description: Description}
    });
...
export class ExampleDataSource extends DataSource<EmployeeType> {
  ...
  connect(): Observable<EmployeeType[]> {
    ...
    this._exampleDatabase.getAllEmployeeTypes();

    return Observable.merge(...displayDataChanges).map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.Employeetypedata.slice().filter((employeetype: EmployeeType) => {
        const searchStr = (employeetype.Id + employeetype.Name + employeetype.Code).toLowerCase();
  ...
  sortData(data: EmployeeType[]): EmployeeType[] {
